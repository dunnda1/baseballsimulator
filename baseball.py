...
class League():
    ...
    def getTeamNameFromAbbrev(self, abbrev):
        a = 1
        name = self.abbrev.loc[self.abbrev['TEAM']==abbrev, 'NAME'].values[0]
        return name

    def init_season(self, teams, schedule=None, year=2017):
        if self.teams is None:
            raise ValueError('No Teams Defined')

        # try to get abbreviations
        abbreviation_file = 'retrosheets/TEAMABR.TXT'
        try:
            current_yr = 2010 
            self.abbrev = pd.read_csv(abbreviation_file, comment='#', header=None, names=['TEAM','LEAGUE', 'CITY', 'NAME', 'FIRST_YR', 'LAST_YR'],keep_default_na=False)
            self.abbrev = self.abbrev[ self.abbrev['LAST_YR']==current_yr]
        except:
            logger.warn(f'WARNING: Unable to read and/or process abbreviations file {abbreviation_file}, proceeding without it but this probably won\'t end well...')
            self.abbrev = None	

        

        if schedule is None:	
            logger.info('No schedule found, creating random schedule')
            self.make_random_schedules()	
        else:
            self.set_schedule(schedule)
        
        self.read_data()

        logger.info('Populating baserunning data')
        self.brDF = fill_baserunning()

        self.standings = pd.DataFrame(index=teams, columns = ['W', 'L', 'GB'])
    ...
    # schedule is a json object gwith team abbreviations pairs (visitor, home) that looks like:
    # [["CHN","SLN"],
    #  ["MIA","WAS"],
    #  ["SDN","LAN"],
    #  ["SEA","HOU"],
    ...
    def make_random_schedules(self, length=24, method="random"):
    ...
    def simulate_season(self):
        ...
        logger.info(f'Starting Season Simulation')
        # lineupfiles = getLineupFiles(list(schedule.keys()))
        ...
        # loop through all games in schedule
        for p in self.schedule:
            # schedule is a list of abbreviated visitor, home pairs using retrostats abbreviations
            awayAbbrev = p[0]
            homeAbbrev = p[1]
            
            homeTeam = self.getTeamNameFromAbbrev(homeAbbrev)
            awayTeam = self.getTeamNameFromAbbrev(awayAbbrev)
